...
from .depth_limited_dfs import DepthLimitedDFS
...
class DepthLimitedDFSWithLLM:
    """
    A graph traversal class that implements predicate-constrained DFS.
    This approach uses an LLM to determine which relationship types are relevant
    for a given query, then filters the DFS traversal to only follow those relationships.
    """
    ...
    def __init__(
        ...
        """
        Initialize the predicate-constrained DFS traversal.
        
        Args:
            max_depth: Maximum depth to traverse (default: 10)
            max_neighbors_per_node: Maximum number of neighbors to consider per node (default: 20)
    ...
    def traverse_graph(self, relevant_docs: List[Dict[str, Any]], user_query: str) -> List[Dict[str, Any]]:
        """
        Retrieve additional context through predicate-constrained DFS traversal.
        
        This method:
        1. Gets all relationship types from the database
        2. Uses LLM to select relevant relationship types for the query
        3. Performs DFS traversal filtering by selected relationship types
        ...
        """
        try:
            print(f"🔍 Starting predicate-constrained DFS traversal")
            ...
            additional_nodes = dfs_traversal.traverse_graph(relevant_docs, user_query)
            
            if additional_nodes:
                print(f"✅ Retrieved {len(additional_nodes)} additional context nodes via predicate-constrained DFS")
            else:
                print("⚠️ No additional context found through predicate-constrained DFS traversal")
            
            return additional_nodes
            
        except Exception as e:
            print(f"⚠️ Error in predicate-constrained DFS traversal: {str(e)}")
            import traceback
            traceback.print_exc()
            return []
    ...
    def get_traversal_statistics(self) -> Dict[str, Any]:
        ...
        """
        return {
            "method": "depth_limited_dfs_pred_llm",
            "max_depth": self.max_depth,
            "max_neighbors_per_node": self.max_neighbors_per_node,
            "max_total_nodes": self.max_total_nodes,
            "remove_mentions_nodes": self.remove_mentions_nodes,
