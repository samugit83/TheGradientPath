...
from .khop_limited_bfs import KhopLimitedBFS
...
class KhopLimitedBFSWithLLM:
    """
    A graph traversal class that implements predicate-constrained BFS.
    This approach uses an LLM to determine which relationship types are relevant
    for a given query, then filters the BFS traversal to only follow those relationships.
    """
    ...
    def __init__(
        ...
        """
        Initialize the predicate-constrained BFS traversal.
    ...
    def traverse_graph(self, relevant_docs: List[Dict[str, Any]], user_query: str) -> List[Dict[str, Any]]:
        """
        Retrieve additional context through predicate-constrained BFS traversal.
        
        This method:
        1. Gets all relationship types from the database
        2. Uses LLM to select relevant relationship types for the query
        3. Performs BFS traversal filtering by selected relationship types
        ...
        """
        try:
            print(f"🔍 Starting predicate-constrained BFS traversal")
            ...
            additional_nodes = bfs_traversal.traverse_graph(relevant_docs, user_query)
            
            if additional_nodes:
                print(f"✅ Retrieved {len(additional_nodes)} additional context nodes via predicate-constrained BFS")
            else:
                print("⚠️ No additional context found through predicate-constrained BFS traversal")
            
            return additional_nodes
            
        except Exception as e:
            print(f"⚠️ Error in predicate-constrained BFS traversal: {str(e)}")
            import traceback
            traceback.print_exc()
            return []
    ...
    def get_traversal_statistics(self) -> Dict[str, Any]:
        ...
        """
        return {
            "method": "khop_limited_bfs_pred_llm",
            "max_hops": self.max_hops,
            "max_neighbors_per_node": self.max_neighbors_per_node,
            "max_total_nodes": self.max_total_nodes,
            "remove_mentions_nodes": self.remove_mentions_nodes,
