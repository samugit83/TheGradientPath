...
from .astar_search_heuristic import AStarSearchHeuristic
...
class AStarSearchHeuristicWithLLM:
    """
    A graph traversal class that implements predicate-constrained A* Search.
    This approach uses an LLM to determine which relationship types are relevant
    for a given query, then filters the A* traversal to only follow those relationships.
    """
    ...
    def __init__(
        ...
        """
        Initialize the predicate-constrained A* Search traversal.
        ...
        Args:
            ...
            model: LLM model to use for relationship selection (default: from params.py)
    ...
    def traverse_graph(self, relevant_docs: List[Dict[str, Any]], user_query: str) -> List[Dict[str, Any]]:
        """
        Retrieve additional context through predicate-constrained A* Search traversal.
        
        This method:
        1. Gets all relationship types from the database
        2. Uses LLM to select relevant relationship types for the query
        3. Performs A* Search traversal filtering by selected relationship types
        ...
        """
        try:
            print(f"🔍 Starting predicate-constrained A* Search traversal")
            ...
            # Get query embedding for A* heuristic
            query_embedding = self.query_embedding
            
            # Initialize AStarSearchHeuristic with relationship filtering
            if selected_relationships:
                print(f"🎯 Using {len(selected_relationships)} selected relationship types for filtering")
                astar_traversal = AStarSearchHeuristic(
            ...
            additional_nodes = astar_traversal.traverse_graph(relevant_docs, user_query)
            
            if additional_nodes:
                print(f"✅ Retrieved {len(additional_nodes)} additional context nodes via predicate-constrained A* Search")
            else:
                print("⚠️ No additional context found through predicate-constrained A* Search traversal")
            
            return additional_nodes
            
        except Exception as e:
            print(f"⚠️ Error in predicate-constrained A* Search traversal: {str(e)}")
            import traceback
            traceback.print_exc()
            return []
    ...
    def get_traversal_statistics(self) -> Dict[str, Any]:
        ...
        """
        return {
            "method": "astar_search_heuristic_pred_llm",
            "max_total_nodes": self.max_total_nodes,
            "remove_mentions_nodes": self.remove_mentions_nodes,
            "model": self.model
        }
