...
class BeamSearchOverGraphWithLLM:
    """
    A graph traversal class that implements predicate-constrained Beam Search.
    This approach uses an LLM to determine which relationship types are relevant
    for a given query, then filters the Beam Search traversal to only follow those relationships.
    """
    ...
    def __init__(
        ...
        """
        Initialize the predicate-constrained Beam Search traversal.
    ...
    def traverse_graph(self, relevant_docs: List[Dict[str, Any]], user_query: str) -> List[Dict[str, Any]]:
        """
        Retrieve additional context through predicate-constrained Beam Search traversal.
        
        This method:
        1. Gets all relationship types from the database
        2. Uses LLM to select relevant relationship types for the query
        3. Performs Beam Search traversal filtering by selected relationship types
        ...
        """
        try:
            print(f"🔍 Starting predicate-constrained Beam Search traversal")
            ...
            # Initialize BeamSearchOverGraph with relationship filtering
            ...
            additional_nodes = beam_search_traversal.traverse_graph(relevant_docs, user_query)
            
            if additional_nodes:
                print(f"✅ Retrieved {len(additional_nodes)} additional context nodes via predicate-constrained Beam Search")
            else:
                print("⚠️ No additional context found through predicate-constrained Beam Search traversal")
            
            return additional_nodes
            
        except Exception as e:
            print(f"⚠️ Error in predicate-constrained Beam Search traversal: {str(e)}")
            import traceback
            traceback.print_exc()
            return []
