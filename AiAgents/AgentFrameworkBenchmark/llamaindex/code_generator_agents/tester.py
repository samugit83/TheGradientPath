"""
Tester Agent using LlamaIndex FunctionAgent pattern
"""
import logging
import subprocess
import sys
...
from .types import TestResults, TestFailure, TestCodeOutput
...
def get_tester_instructions(main_state: MainState) -> str:
    """
    Dynamic instructions for the Tester agent based on current state.
    ...
    if state.current_code:
        code_to_test = state.current_code.code
    elif state.previous_code:
        code_to_test = state.previous_code
    else:
        main_file = Path.cwd() / state.package_name / "main.py"
        if main_file.exists():
            code_to_test = main_file.read_text(encoding='utf-8')
    
    formatted_prompt = TESTER_PROMPT_TEMPLATE.format(
        code_to_test=code_to_test,
        max_test=state.max_tests
    )
...
def create_tester(main_state: MainState) -> FunctionAgent:
    """
    ...
    instructions = get_tester_instructions(main_state)
    
    agent = FunctionAgent(
        name="Tester Agent",
        description="An expert Python testing specialist that generates comprehensive unit tests",
...
def post_process_test_output(
    ...
    This saves the test code, runs the tests, and updates the state.
