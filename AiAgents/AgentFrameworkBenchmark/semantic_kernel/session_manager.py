...
class MessageRole(Enum):
    """Enum for message roles in conversation - aligned with Semantic Kernel AuthorRole"""
    USER = "user"
    ASSISTANT = "assistant"
    SYSTEM = "system"
    TOOL = "tool"
...
class Message:
    ...
    Aligned with Semantic Kernel conversation format.
    """
    role: MessageRole
    content: str
    timestamp: str = field(default_factory=lambda: datetime.now().isoformat())
    ...
    # Agent information (for easier Semantic Kernel integration)
            ...
            with sqlite3.connect(self.db_path) as conn:
                ...
                cursor.execute("""
                    INSERT INTO messages (
                        session_id, message_id, role, content, timestamp, agent_name,
    ...
    def get_messages(self, limit: Optional[int] = None, offset: int = 0) -> List[Message]:
        ...
        try:
            with self.lock:
                ...
                query = """
                    SELECT message_id, role, content, timestamp, agent_name, tool_calls,
